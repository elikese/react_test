...
function Mypage() {
    ...
    useEffect(() => {
        ...
        if (userData !== null) {
            ...
            setBirthdayValue(userData.birthday);
            setProfileUrl(userData.imgUrl);
        }
    ...
    return (
        <div css={S.layout}>
            <div css={S.imageBox}>
                <input type="file" style={{ display: "none" }} ref={fileRef} onChange={handleInputChange} />
                <img src={profileUrl} alt="profile" onClick={() => { fileRef.current.click() }} />
            </div>
            ...
            <WideButton text={"수정하기"} onClick={handleEditClick} />
        </div>
    );
}
